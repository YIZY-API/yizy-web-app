...
export function serviceToOpenApiSpec(service: YIZY.Service, outputType: OpenAPISpecType): string {
	const serviceName =
	...
	service.endpoints.forEach((e: YIZY.Endpoint) => {
		...
		if (!e.requestModel) {
			...
			if (YIZY.isObjectType(e.requestModel)) {
				req =
					typeof (e.requestModel as YIZY.ObjectType).name === 'string'
		...
		if (!e.responseModel) {
			...
			if (YIZY.isObjectType(e.responseModel)) {
				res =
					typeof (e.responseModel as YIZY.ObjectType).name === 'string'
		...
		doc.addPath({
			url: e.url,
			operationId: typeof e.name === 'string' ? e.name : (e.name as YIZY.NameMap).default,
			requestRef: req,
			responseRef: res
		});
	});
	...
	service.referenceTypes.forEach((obj: YIZY.ObjectType) => {
		doc.addComponent({
			name:
				typeof obj.name === 'string' ? (obj.name as string) : (obj.name as YIZY.NameMap).default,
			properties: obj.fields
		});
	});
...
export class OpenApiDoc {
	...
	addPath(path: AddPathDto) {
		...
		if (path.requestRef != null) {
			...
			this.doc['paths'][path.url]['post']['requestBody']['content']['application/json'] = {};
			this.doc['paths'][path.url]['post']['requestBody']['content']['application/json']['schema'] =
				{};
		...
		if (path.responseRef != null) {
			...
			this.doc['paths'][path.url]['post']['responses']['default']['content']['application/json'] =
				{};
			this.doc['paths'][path.url]['post']['responses']['default']['content']['application/json'][
				'schema'
			] = {};
			...
			this.doc['paths'][path.url]['post']['responses']['default']['description'] =
				'default response';
		} else {
			this.doc['paths'][path.url]['post']['responses'] = {};
			this.doc['paths'][path.url]['post']['responses']['default'] = {};
	...
	addComponent(comp: AddComponentDto) {
		...
		comp.properties.forEach((prop) => {
			...
			const n =
				typeof prop.name === 'string' ? (prop.name as string) : (prop.name as YIZY.NameMap).default;
	...
	private fieldDataTypeToProperty(type: YIZY.FieldDataType) {
		...
		if (YIZY.isPrimitiveType(type)) {
			...
			if (YIZY.isArrayType(type) || YIZY.isNullableArrayType(type)) {
				...
				obj['items'] = {};
				if (YIZY.isNullable(type)) {
					obj['items'] = this.fieldDataTypeToProperty((type as YIZY.NullableArrayType).itemType);
				} else {
					obj['items'] = this.fieldDataTypeToProperty((type as YIZY.ArrayType).itemType);
				}
			}
			if (YIZY.isRefType(type) || YIZY.isNullableRefType(type)) {
				if (YIZY.isNullable(type)) {
					obj['$ref'] = '#/components/schemas/' + (type as YIZY.NullableReferenceType).ref;
				} else {
					obj['$ref'] = '#/components/schemas/' + (type as YIZY.ReferenceType).ref;
				}
			}
		}
		return obj;
